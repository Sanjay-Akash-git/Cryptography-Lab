
import binascii
from collections import Counter


hexd_messages = [
"0f381a39fe6f41bd57c44646eacc3ecb2b695ae729ee174ac650ab0c92547a73b19ca7a24d40162bea9c0d1c2c1395678f6dec2ae8b4eaa449b1511507c3e06dd6c9c02c69c5eca4241d3f3585f44440ad011078381bacc075e4c3",
"0f351c71d96e59b742c34053a6853d9930664da237f24456874ae61198546b7ea6c8f7a34b581823ead8490c29568e618b68a929f3e6e6ea0ca35f1944c2ec70d686df3028c4f9a42a0720748cbb4e41a74c07773213bad56eefde922d44cdbcbf3e008be80870a5eb7c55ab907f18fcf347dd433bcf83432d0849e80c22b0",
"",
"",
"",
"",
"",
"",
"14331c71fd614eba59c34007e58d2099206108f632f81755c851e04198403f79a1daa3a902590d2be6964c1b26138f6b95258228a7abeee744be591944c9e46d828dc2697cc3faa4351d3f7ec2f44b0ea54f17393e08afd366efc2d63743c2ada33e1982e3",
"",
"",
"",
"0f351c71c7654ff251de056ea68920cf2d7d49e53ff9174bd303fc04d74e7e69ad9cb9bf56160c2aea960d002b139b6b8f25982fe2e6e9f158a2451944c3f623d19dc425648beceb621f38768abb4f47ad46176b7b04b3c069a0c4da3b0dd3bea96a54b7e4453989f86b55ba8b635b90f944"
]

messages = [binascii.unhexlify(h) for h in hexd_messages]
num = len(messages)
m_len = max(len(m) for m in messages)

key = [None] * m_len

def xor_bin(a, b):
    return bytes(x ^ y for x, y in zip(a, b))


def spacevotes(threshold=3):
    vote = [Counter() for _ in range(m_len)]
    for i in range(num):
        for j in range(i+1, num):
            ln = min(len(messages[i]), len(messages[j]))
            x = xor_bin(messages[i][:ln], messages[j][:ln])
            for pos, val in enumerate(x):
                c = val ^ 0x20
                if (65 <= c <= 90) or (97 <= c <= 122):
                    vote[pos][i] += 1
                    vote[pos][j] += 1
    for pos in range(m_len):
        if not vote[pos]:
            continue
        idx, cnt = vote[pos].most_common(1)[0]
        if cnt >= threshold and key[pos] is None and pos < len(messages[idx]):
            key[pos] = messages[idx][pos] ^ 0x20


def crib_words(words):
    for w in words:
        crib = w.encode()
        for ci, ctext in enumerate(messages):
            for start in range(len(ctext) - len(crib) + 1):
                kfrag = bytes([ctext[start+i] ^ crib[i] for i in range(len(crib))])
                good = True
                for i in range(len(crib)):
                    pos = start + i
                    if key[pos] is not None and key[pos] != kfrag[i]:
                        good = False
                        break
                if good:
                    for i in range(len(crib)):
                        pos = start + i
                        if key[pos] is None:
                            key[pos] = kfrag[i]

spacevotes()
crib_words([" the ", " and ", " to ", " of ", " is ", " web ", " future ", " past "," The ", " and ", " ing ", " to ", " in ", " is ", " for ", " that ",
        " with ", " envisage ", " be ", "not ", " you ", " bigger", " message",
        " we "," as "," it ", " future ", " seen ", " video "," than ","  your ", " than ", " bigger ", " web "," important "," live "," please ", " much ", " yet ",
        " so ", "technological ", "user ", "server ", "error ", " call ", " lot ", " machine "])
spacevotes(threshold=2)

for idx, ciphertext in enumerate(messages):
    dec = []
    for i, b in enumerate(ciphertext):
        if key[i] is None:
            dec.append("_")
        else:
            ch = b ^ key[i]
            dec.append(chr(ch) if 32 <= ch < 127 else "_")
    print(f"Message {idx+1}:", "".join(dec))
    print()
